...
class Hydrator(object):
    ...
    def solve(self,inputs):
        self.deltaTmin_SSHX = inputs["min_temperature_exchange"]+5  #固-固换热器最小温差
        self.deltaTmin_SGHX = inputs["min_temperature_exchange"]  #固-气换热器最小温差
        results = {}
        ...
        #HEN 
        self._T_delta_pinch = inputs["min_temperature_HEN"] 
        c = self.find_optimal_m2(results)
        self._pinch_point_data, hot_in, hot_hen, cold_hen, exergy_hen = self.pinch_point_data(results, c)
        ...
        results["Hydr"]["HEN"]["total_HEN_area"]=total_HENA/self._heat_transfer_loss_eff
        results["Hydr"]["HEN"]["c"] = c
        ...
        results["Hydr"]["flue_gas_mass_flow"]=results["Brayton"]["evaluation_indicators"]["flue_gas_mass_flow"]+c
        # steam blower
        steam_name = "water"
        steam_pi = self._P_hydr
        steam_po = self._P_amb/(1-self._steam_pressure_loss_ratio)
        steam_mass_rate = results["Hydr"]["hydrator"]["mass_steam_in"]
        steam_blower = self.steam_blower_power(self._T_hydr,
                                         steam_pi,
                                         steam_po,
                                         steam_mass_rate,
                                         steam_name)
        results["Hydr"]["steam_blower"] = steam_blower
        # conveying power
        results["Hydr"]["conveying_power"] = self.conveying_power(
            results["Hydr"]["hydrator"]["m_camix_in"],
            results["Hydr"]["hydrator"]["m_camix_out"])*(-1)
        flue_gas_name = self._pw.get_flue_gas_refprop_name()
        results["Hydr"]["flue_gas_fan_power"]=self.flue_gas_fan_power(self._industrial_waste_heat_t,
                                                            self._P_amb,
                                                            self._P_amb/0.99,
                                                            results["Hydr"]["flue_gas_mass_flow"],
                                                            flue_gas_name)
        #Case all
        B_case = self.B_case(results,
                             results["Hydr"]["HEN"]["hot_in"],
                             results["Hydr"]["HEN"]["hot"],
                             results["Hydr"]["HEN"]["cold"])
        results["Case_B"] = B_case
        All_case = self.All_case(results)
        results["Hydr"]["cost"] = self.cost(results)
        results["Case_All"] = All_case
        
        return results
    
    def find_optimal_m2(self, results, tolerance=1e-2):
        a = 0.5
        b = 4*self._Store_electrical_power/1e6
        while b - a > tolerance:
            mid = (b + a) / 2
            c = mid
            self._pinch_point_data, hot_in, hot_hen, cold_hen, exergy_hen = self.pinch_point_data(results, c)
    ...
    def pinch_point_data(self,res,m2):
        self._materials=["Ca","Gas","Gas","Ca","Water","Water","Gas"]
        ...
        exergy["cold_add"]=(exergy["PC_water"]+exergy["C_CaoM"]+
                            exergy["be_PCwater"]+exergy["C_Steam"])
        exergy["lost"]=exergy["hot_in"]-exergy["cold_add"]

        return pinch_point_data,hot,a,b,exergy
    ...
    def B_case(self,results,hot_in,hot_hen,cold_hen):
        b={}
        b["power_in"]=-results["Hydr"]["steam_blower"]["power"]-results["Hydr"]["conveying_power"]-results["Hydr"]["flue_gas_fan_power"]
        ...
        b["energy_eff_hydr"]=((results["Brayton"]["evaluation_indicators"]["re_Heat_in"])
                              /(b["power_in"]+b["hot_in_hydr"]+results["Caes"]["hot_stockpile"]))
        b["energy_eff"]=(b["power"]-b["power_in"])/(b["hot_in"]+results["Caes"]["hot_stockpile"])
        return b
    
    def cost(self,results):
        result={}
        result["cost_hen"]=3197*(results["Hydr"]["HEN"]["total_HEN_area"]**0.67)
        result["cost_hydr"]=19594*(results["Hydr"]["hydrator"]["Q_produce"]/1000)**0.5
        result["cost_steam_blow"]=750*((-results["Hydr"]["steam_blower"]["power"]/1000)**0.71)*(1+0.2/(1-self._isentropic_eff_mc))
        result["cost_flue_gas_fan"] = 0.827*1000000/6.901*(-results["Hydr"]["flue_gas_fan_power"]/445000)**0.67
        
        result["cost_all"]=result["cost_hen"]+result["cost_hydr"]+result["cost_steam_blow"]+result["cost_flue_gas_fan"]
        return result
