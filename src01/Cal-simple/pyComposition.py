...
class Hydrator(object):
    ...
    def solve(self,inputs):
        ...
        self._t= inputs["T_X"]
        self._reactants_ES_mole = self._cao_conversion
    ...
    def high_tem_HEN(self,X):
        ...
        cp_caco3_i = self._pw.cp0mass_mean("caco3", self._T_hydr-self.deltaTmin_SSHX, self._T_amb)
        cp_caco3_o = self._pw.cp0mass_mean("caco3", self._T_hydr, self._T_amb+self.deltaTmin_SSHX)
        h_steam_in = CP.PropsSI('H', 'T', self._T_hydr-self.deltaTmin_SGHX+273.15, 'P', self._P_amb, "REFPROP::co2")
        h_steam_out = CP.PropsSI('H', 'T', self._T_amb+273.15, 'P', self._P_amb, "REFPROP::co2")

        a=cp_caco3_o*M_caco3*(self._T_hydr-self._T_amb-self.deltaTmin_SSHX)
        b_s=(cp_caco3_i*M_caco3*(1-X))+(cp_cao_i*M_cao*X)#冷
        b_g=(h_steam_in-h_steam_out)*M_co2
        t_s_out = self._T_amb+(a*0.96-b_g)/b_s
        ...
        if t_s_out> a1:
            ...
            t_s_out=t_s_out
        h_mole_lost=(a-(t_s_out-self._T_amb)*b_s-b_g)
        return t_s_out,h_mole_lost
    ...
    def hydrator(self,mole_re):
        ...
        results["mole_caco3_o"] = mole_re/self._cao_conversion
        results["mass_caco3_o"] = mole_re/self._cao_conversion*M_caco3
        results["mole_cao_in"] = mole_re
        results["mass_cao_in"] = mole_re*M_cao
        results["mole_caco3_in"] = (mole_re/self._cao_conversion)-mole_re
        results["mass_caco3_in"] = results["mole_caco3_in"]*M_caco3      
        results["mole_steam_in"] = mole_re
        results["mass_steam_in"] = mole_re*M_co2
        results["m_camix_in"]= results["mass_cao_in"]+results["mass_caco3_in"]
        results["m_camix_out"] = results["mass_caco3_o"]

        heat_cao = (self._pw.cp0mass_mean("cao", self._T_hydr,self.Hydr_cao_in )
                    *(self._T_hydr-self.Hydr_cao_in)*results["mass_cao_in"]+
                    self._pw.cp0mass_mean("caco3", self._T_hydr, self.Hydr_cao_in )
                    *(self._T_hydr-self.Hydr_cao_in)*results["mass_caco3_in"])
        heat_steam=((CP.PropsSI('H', 'T', self._T_hydr+273.15, 'P', self._P_hydr, "REFPROP::co2")
                    -CP.PropsSI('H', 'T', self.Hydr_steam_in+273.15, 'P', self._P_hydr, "REFPROP::co2"))
