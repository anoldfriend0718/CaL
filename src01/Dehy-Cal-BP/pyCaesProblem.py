...
from pyComposition import Hydrator
import pandas as pd
# global parameters
T_amb = 20
p_amb = 101325
...
class CaesProblem(ea.Problem):
    ...
    def _compose_generic_algorithm_paramters(self):
        ...
        lb = [17794921.326326743,12258207.576473976,1e5,300,4,20,30e6,16217752,12217752,1e5,0.37,650,214]  
        # 12热泵高压，中间压，3氢氧化钙入反应器温度,45储压力过热
        #678布雷顿高压，高低温中间压，910释压力过热,11氧化钙入反应器温度
        #12储能侧供暖水流量、13释能侧补烟气
        ub = [17794921.326326743,12258207.576473976,1e5,500,10,20,30e6,16217752,12217752,1e5,0.37,650,214] #  上界
    ...
    def evalVars(self, Vars):  # 目标函数
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=self._max_map_executor) as map_executor_pool:
            ...
            for i in np.arange(0,case_num,1):
                ...
                inputs["p_Dehy"]=Vars[i,2]
                inputs["t_HEN"] = Vars[i,3] #变量4，反应器压力
                inputs["m_water"] = Vars[i,4] #变量2，脱水反应器过热温度
                inputs["Dehy_overheating_temperature"] = Vars[i,5]

                inputs["p_bray_H_B"] = Vars[i,6]
                inputs["p_bray_MH_B"] = Vars[i,7]
                inputs["p_bray_ML_B"] = Vars[i,8]

                inputs["p_Hydr"] = Vars[i,9]
                inputs["R"] = Vars[i,10]
                inputs["HT"] = Vars[i,11]
                inputs["H_out"] = Vars[i,12]#补充烟气
            ...
            for _, future in enumerate(futures):
                ...
                c2 = results["Dehy"]["HEN"]["hot_utility"]-self._hot_util
